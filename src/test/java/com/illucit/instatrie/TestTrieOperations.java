...
public class TestTrieOperations {
     ...
     * equally distributed.
     *
     * @param prefix       fixed prefix (optional)
     * @param minVarLength minimum length of veriable string part
     * @param maxVarLength maximum length of veriable string part
     * @param minChar      minimum char in Unicode range
     * @param maxChar      maximum char in Unicode range
     * @return random word under the given preconditions
     * @throws IllegalArgumentException if the preconditions are invalid (min > max)
     */
     ...
     * Run test suite.
     *
     * @param verbose print debug messages to STDOUT/STDERR, if true
     */
    private void runTestSuite(boolean verbose) {
        ...
        for (String candidate : posAndNeg) {
            ...
            assertTrue(contained == shouldBeContained);
        }
        if (verbose) {
            end_date = new Date().getTime();
        ...
        for (String candidate : unfoundPrefixes) {
            assertFalse(trie.containsPrefix(candidate.toCharArray()));
            foundNegCandidates++;
        }
        if (verbose) {
            end_date = new Date().getTime();
